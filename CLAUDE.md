...
## Weekly Athletic Profile (latest — 2026-09-12)

*Overwritten by each weekly athletic-profile/objectives-tracker run. Full report: `profile-reports/2026-09-12-athletic-profile-objectives.md`. The daily routine should treat this as standing context alongside `LAST_STATE`, not re-derive it.*

- **Classification**: detrained sub-elite masters endurance athlete (half-marathon PR 1:34:33, marathon PR 3:39:55, both 2019, both top 5-10%/30-35% of French amateur fields) in a medically-supervised, GLP-1-assisted re-training phase. VO2max running 42.2 (ACSM "Good", ~75th pct for age/sex, currently rebounding) is discordant with Class I obesity (BMI ≈ 33) — cardiorespiratory fitness survived the 2020-2025 detraining far better than body composition did.
- **Current comeback volume** (since 2026-04-26): 2.15 sessions/wk, 1.35 h/wk — running-led for the first time in the 12-year record, but still well below both the 3-runs/wk objective and the historic peak-era norm (4.5-5.6 sessions/wk, 2014-2019).
- **Objectives confidence (0-100, prob. of hitting target_date), all down vs 2026-09-05**: root "retour compétiteur" 14% (was 17%) · 3 runs/wk 12% (16%) · protein ≥160g/j 3% (5%) · 95 kg by Dec 2026 22% (28%) · 12 km Beaumonts 15% (20%) · blood panel 20% (24%) · semi <1h45 8% (10%).
- **Critical-path bottleneck (unchanged, worsening)**: nutrition logging has been at **zero rows for 20 consecutive days** (Aug 23–Sep 11) — not just zero macros, no logging at all. On an appetite-suppressing GLP-1 this is a genuine under-fuelling / lean-mass risk regardless of true intake, and is the probable gate on HRV/RHR normalization. Highest-leverage action: restart logging today; 3 consecutive days ≥120 g protein is the trigger to re-check the deload gate, rather than waiting on HRV/RHR alone.
- **Live confound, exculpatory**: a symptom-log entry "Gros rhume" (cold, started 2026-09-08, still active as of the refresh) explains the Sep 8-9 RHR spike (57-58 bpm) and body-battery crash (20-21) as a resolving illness, not a deepening recovery deficit or a GLP-1 escalation effect — Sep 11 showed the best readiness numbers in three weeks (RHR 51, HRV 34, body battery 61). Treat any lingering RHR/HRV elevation in the days immediately after as illness recovery, not a fresh red flag, unless it persists past ~1 week post-onset.
- **Weight/dose interaction to watch**: demonstrated weight-loss pace (-0.205 kg/wk) is currently *below* the low end of the GLP-1-expected band (0.5-1.0%/wk), i.e. the medication looks like it's under-delivering, not overshooting — worth cross-checking against whether the 2026-09-09 dose step to 0.5 mg was actually confirmed (still unconfirmed either way; no injection ever logged in `oh_supplement_logs`).
- **12 km Beaumonts long-run ceiling**: still 10.5 km, 7 weeks stalled — do not raise the prescribed long-run target past demonstrated tolerance until a fresh attempt lands.
...
- **Weekly athletic-profile / objectives tracker**: `profile-reports/YYYY-MM-DD-athletic-profile-objectives.md` — a separate, weekly (not daily) deep dive: lifetime Strava/Withings/Garmin classification plus a live refresh of every `objectives` row (current_value, notes, confidence score), written back to the `objectives` table via `upsert_objective` each run. **Read the latest file in `profile-reports/` at Phase 0 alongside `LAST_STATE`** — it carries the current objective confidence scores, the identified critical-path bottleneck, and any medication/adherence context (e.g. a live dose-escalation or illness confound) that should inform today's plan. See the "Weekly Athletic Profile (latest)" section below for the condensed version.
