...
| Expected escalation | Wegovy label titration is 0.25 → 0.5 → 1.0 → 1.7 → 2.4 mg with 4 weeks per step, so **2026-09-09 is the expected 0.5 mg step**, then ~07/10 (1.0), ~04/11 (1.7), ~02/12 (2.4) if the prescriber follows the label. Each confirmed step restarts the RHR/HRV rebaseline clock and opens a `T.glp1.dose_escalation_watch_days` window (GI effects and RHR offset are dose-dependent). **Confirm each step with Benjamin; never assume it happened.** — **Status as of 2026-09-15 (monthly maintenance pass): the 2026-09-09 date has now passed, 6 days ago, still unconfirmed** — no matching entry exists in `oh_supplement_logs` either way. Garmin RHR spiked to 57–58 bpm on 2026-09-08/09 (vs a 51–55 bpm recent baseline) then settled back to 51–53 bpm by 2026-09-13 — a plausible but **unconfirmed** early signal consistent with the expected step, not evidence it happened. `T.glp1.current_dose_mg` is unchanged at 0.25 mg pending Benjamin's confirmation. | SmPC + consult ("chaque mois on réévalue … même dosage ou on ajuste") — medium for dates |
...
- [ ] Dose escalation dates as they happen (first expected 2026-09-09 → 0.5 mg, **now 6 days overdue for confirmation as of 2026-09-15** — ask Benjamin directly); update `T.glp1.current_dose_mg` / `dose_history` only once confirmed
